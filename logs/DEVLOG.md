...
### Current state
The transfer task completes reliably enough to call this a real milestone, but not every run succeeds — the dominant remaining failure mode is still the shelf-1-exit soft-cost-zone issue described above (some runs churn on proactive replans or exhaust backoff attempts before ever reaching the staging point). A live-observed anomaly (a chassis turn that looked like it went 270° instead of the expected -90°) is flagged but **not confirmed or refuted** — no fine-grained trajectory logging exists yet to check it after the fact, and the turn-direction math was checked by static read and looks correct (`atan2(sin,cos)`, which by construction always picks the shorter rotation). All of today's code changes are uncommitted going into this update; `simulation/models/` and `simulation/reward_logs/` are untracked files unrelated to today's work (looks like separate RL experimentation, includes a 173MB checkpoint) and were deliberately left out of the commit.

### Next steps (high level)
1. If pursuing full reliability: push the shelf-1 exit point genuinely clear of the *soft* cost radius (~0.87m), not just the lethal one (~0.37m) — the clearly-identified remaining lever.
2. Confirm or refute the "270° turn" observation with real trajectory logging next time it's suspected live.
3. Fix the backoff mechanism's retreat-direction unreliability (still unresolved, just exercised less often now).
4. Decide what to do with `simulation/models/`/`simulation/reward_logs/` (separate from this session's work, currently untracked and uncommitted).
5. Carried over: `sudo dmesg` after a future crash, phase-two friction-based holding, the pygame transport-negotiation port, RL decision layer, un-version-controlled robot asset fixes.
