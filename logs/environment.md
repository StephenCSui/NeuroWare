...
### Current state
Working. Season rebalanced (shorter maturity, much longer/safer harvest window), a global time-compression constant added, difficulty deliberately reintroduced via bed count + decay/weed multipliers, and two new small diagnostic logs (reward, death-cause) built and wired in. Current difficulty tuning is lopsided (94% of deaths are dehydration, see below) — flagged, not yet rebalanced.

### What was done
- **`DAYS_TO_MATURE` 28→20**, `SEASON_LENGTH_DAYS` unchanged at 30 — harvest window widened from 2 days to 10, per explicit request. This alone wasn't enough to fix the old "ready cells can still die" problem — see the `Cell._is_active()` change below, which is what actually made the harvest window safe.
- **`Cell._is_active()` narrowed from `status in ("growing", "ready")` to `status == "growing"` only.** This is the real behavioral change, not just the day-count retune: a `"ready"` cell no longer drifts (`Cell.tick()` no-ops for it, same mechanism terminal cells already used), no longer shows `needs_water`/`needs_weeding`/`needs_monitoring`, and — since `Grid.tick()`'s death-check loop was narrowed to `cell.status == "growing"` too — can no longer die from neglect during the harvest window at all. Only `needs_harvest()` (unchanged, reads `status` directly, not gated by `_is_active()`) stays true for a ready cell.
- **Death-cause classification added to the same `Grid.tick()` death-check branch:** each death now resolves to one of `"dehydration"` (moisture-only trigger), `"weeds"` (weed-only trigger), or `"both"` (either both hard thresholds fired together, or only the softer combined `moisture<=20 and weed>=80` threshold fired) and is recorded via `self.death_log.record(...)`.
- **New `TIME_SCALE = 4.0` constant** (`config.py`) — `AGENT_SPEED`, `BUSY_CHANCE`/`BUSY_DURATION`, `MOISTURE_DECAY_BASE`, `WEED_SPROUT_CHANCE`, all `TASK_DURATIONS` entries, `STALENESS_THRESHOLD_TICKS`, `CLAIM_TIMEOUT_TICKS`, `DAY_LENGTH_TICKS`, and `STUCK_REROUTE_FRAMES` all now derive from it (multiplied or divided as appropriate to preserve relative balance — e.g. a day is 4x fewer ticks, but decay-per-tick is 4x higher, so decay-per-day is unchanged). Deliberately does **not** speed up headless training — that cost is dominated by the RL policy's batch refit over its replay buffer (see `agent.md`'s RL section), which doesn't shrink with tick count.
- **Difficulty deliberately reintroduced**, since the widened harvest window above pushed the random baseline to ~94% harvest rate — see `DEVLOG.md` for why that's a problem for RL evaluation specifically. `PLANT_REGION` doubled from `(3,2,3,2)` (6 cells) to `(3,2,4,3)` (12 cells) — this turned out to be the *dominant* difficulty lever, more than the multiplier below. New `DIFFICULTY_MULT` constant (tried 1.5, landed on 1.3 — see `DEVLOG.md`'s data) multiplies `MOISTURE_DECAY_BASE` and `WEED_SPROUT_CHANCE` on top of `TIME_SCALE`.
- **New `WATER_CAPACITY = 2` constant** — see `agent.md` for the `Agent.water`→`water_charges` conversion this drives.
- **Two new files, both gitignored generated-output logs** (`simulation/reward_logs/`, directory already gitignored from the prior session's `reward_log.py`): `death_log.py` (`DeathLog` — mirrors `RewardLog`'s shape: `record()`/`summary()`/`save()`-to-CSV). `Grid.__init__` gained `self.death_log = DeathLog()` alongside the existing `self.reward_log`. `main.py` saves both on quit and prints a summary to console.
- **`grid.agents` — new plain attribute, set externally** (`main.py`/`train.py`, right after building the agent list: `grid.agents = agents`), not a `Grid.__init__` field. Needed by two things added this session: the RL swarm-context observation block (`rl_policy.py`) and the deadlock fix's `_find_agent` lookup (`agent.py`) — both need to see the *other* live `Agent` objects, which `Grid` otherwise has no reference to.

### What worked
- Full lifecycle reverified headless after the `_is_active()` change: 0 anomalies across a full season (no `"ready"` cell ever showed a water/weed/monitor need), first cell reliably went `"ready"` on exactly day 20.
- `TIME_SCALE` balance preservation confirmed empirically after the `_move_toward` fix (see `agent.md`) — 604.0 avg score / 225 harvested / 14 dead over 40 episodes, matching the pre-`TIME_SCALE` 603.4 / 224 / 16 almost exactly, despite running in 1/4 the ticks.
- `death_log` worked immediately and surfaced a real, previously-invisible imbalance — see `DEVLOG.md`'s "What worked".

### What did not work
- See `agent.md` for the `_move_toward` oscillation bug — introduced by this session's own `TIME_SCALE` change (`AGENT_SPEED` outgrew the old fixed arrival threshold), caught and fixed within the same session.
- `DIFFICULTY_MULT=1.5` (with the 12-bed change) produced a 71% death rate — judged too harsh, likely past the point where decision quality could still matter (see `DEVLOG.md`). Dialed to 1.3, which only moved the needle to ~68% death rate, revealing the bed-count doubling (not the multiplier) is what's actually driving the difficulty. Not further resolved — an open tuning question for the next session.

### Key decisions
- Bed-count doubling and the `DIFFICULTY_MULT` decay/weed multiplier were treated as two separate, independently-tunable levers on purpose (rather than one combined "difficulty" knob) — this is exactly what let the 1.5→1.3 test isolate which one actually matters, which wouldn't have been possible with a single conflated constant.
- `WATER_CAPACITY` and the death-cause log are related in intent but built independently: the capacity change was meant to fix a suspected water/weed imbalance pre-emptively; the death log was built to actually *measure* whether that suspicion is correct. It turned out the suspicion was directionally right but the fix wasn't enough on its own (94% of deaths are still dehydration) — a good example of why the measurement tool mattered as much as the fix attempt.
- `grid.agents` was added as a plain externally-set attribute rather than a constructor parameter — `Grid` is built before the agent list exists (agents need spawn cells from the grid first), so there's an unavoidable ordering dependency; documented clearly at the two call sites (`main.py`, `train.py`) rather than restructuring construction order.

### Dependencies
- `agent.py`: `_should_yield`/`_find_agent` now read `grid.agents` and `Cell.claim_tick` — see `agent.md` for the full deadlock-fix dependency chain.
- `rl_policy.py`: `_swarm_features` reads `grid.agents`; `N_CELLS` (now 12, was 6) and `FEATURE_DIM`/`ACTION_DIM` derive from `PLANT_REGION`'s cell count — these are *not* auto-computed from `config.PLANT_REGION`, they're a separate hardcoded constant in `rl_policy.py` that must be kept in sync manually if `PLANT_REGION`'s cell count changes again.
- `main.py`/`train.py`: both now set `grid.agents = agents` immediately after building the agent list — required for the two dependents above to work in either live or headless/training contexts.

### [WORTH EXPLORING]
- `rl_policy.py`'s `N_CELLS` constant not being derived automatically from `config.PLANT_REGION` (width × height) is a real footgun — anyone resizing the plant region again will get a silent shape mismatch (or a hard crash) unless they remember to update it manually in a second file. Worth fixing by computing it from `PLANT_REGION` directly next time this file is touched.
- The current difficulty tuning (12 beds, 1.3x decay/weed multiplier, 2 water charges) produces a ~94%-dehydration/2%-weeds/4%-both death split — not a balanced multi-threat environment. Whether to rebalance before or after the next RL training run is an open decision for the user (see `DEVLOG.md`'s next steps).

## Session 7 — NeuroWare (2026-07-19)

### Current state
Working. Scoring is no longer purely additive — `DEATH_PENALTY` and `DEHYDRATION_BLEED_RATE` (both `config.py`) give `grid.score` (and, via `Agent.pending_penalty`, the RL reward stream — see `agent.md`) its first negative terms. A new `"bleeding"` cell color makes the dehydration-bleed state visible live. `eval_harvest_rate.py` is a new standing diagnostic script alongside `train.py`. Death-cause split re-verified unchanged at ~95%/2%/3% dehydration/weeds/both.

### What was done
- **`DEATH_PENALTY = -50.0`** (`config.py`) — applied once per cell death in `Grid.tick()`'s existing death branch: `self.score += DEATH_PENALTY`, plus `agent.pending_penalty += DEATH_PENALTY` for every agent in `grid.agents` (team-level, not attributed to a specific agent — see `agent.md` for why that's actually fine given the shared-policy architecture).
- **`DEHYDRATION_BLEED_THRESHOLD = 40.0` / `DEHYDRATION_BLEED_RATE = -1.0`/sec** (`config.py`) — new check added to the *top* of `Grid.tick()`'s per-cell growing-phase branch (before the existing death-condition checks, not replacing them): any `"growing"` cell at or below 40 moisture (i.e. >60% dehydrated) bleeds `DEHYDRATION_BLEED_RATE * dt` off `grid.score` and every agent's `pending_penalty`, every tick, for as long as it stays under threshold. Rate is defined per *second*, not per tick, specifically so it stays meaningful independent of `FPS`/`TIME_SCALE` — this was a deliberate correction mid-session, see "What did not work."
- **`COLORS["bleeding"]` + `Cell.color()` override** (`config.py`/`grid.py`) — any growing cell at/below the bleed threshold renders flat yellow, overriding both the normal healthy/needs-water/weedy gradient and the "unknown" (never-observed) grey masking. Explicitly a human-observability aid, not a simulation of what the swarm itself knows (the swarm's actual knowledge is still gated by `known_moisture`, see `agent.md`).
- **Built `eval_harvest_rate.py`** (repo root of `simulation/`, new file) — see `DEVLOG.md` for what it does and why it exists (score isn't survival rate). Read-only relative to the environment (loads a saved policy, runs N episodes, tallies `grid.stats()["status_counts"]`), no `grid.py`/`config.py` changes required to support it — it reuses entirely existing stats/status-count machinery from Session 6.
- **`.gitignore`** gained `simulation/reward_logs/` — the reward-log output directory (`reward_log.py`, Session 6) was writing generated CSVs that weren't yet excluded.

### What worked
- **Smoke-testing reward-magnitude changes with a 5-episode headless run before committing to a full training run** — caught the 10x-too-strong bleed rate (see below) in seconds rather than after a wasted training run, and confirmed the rescaled version landed in a sane range (-175 to +160 across 5 random-baseline seasons) before spending compute on it.
- **The `"bleeding"` visual indicator immediately surfaced a real design gap** (see "What did not work") that headless stats alone hadn't shown — a concrete case of live play-testing catching something a pure numbers-only workflow would have missed.
- **Death-cause re-verification** (50 episodes, final model): 94.8% dehydration / 2.2% weeds / 3.1% both — confirms the Session 6 imbalance is stable and not an artifact of the old difficulty tuning; it persisted through a full round of reward-shaping changes that didn't touch weed mechanics at all, exactly as expected.

### What did not work
- **`DEATH_PENALTY` alone regressed the trained policy below random** — see `agent.md`/`DEVLOG.md` for the full root-cause (a `pending_penalty` credit-assignment bug, not an environment-tuning issue). Listed here too because the fix (`DEHYDRATION_BLEED_*`) is an environment/config-level change.
- **First `DEHYDRATION_BLEED_RATE` value (-0.1, applied per raw physics tick) was roughly 10x too strong** — measured, not assumed: 5-season random-baseline smoke test averaged -400 to -700 total score, versus a maximum possible positive pool around +1200 (12 cells × `HARVEST_REWARD`). Root cause was unit confusion, not magnitude alone — `MOISTURE_DECAY_BASE` produces fast enough decay (~2.6/tick at current `TIME_SCALE`/`DIFFICULTY_MULT`) that cells dip below the 40 threshold often, and a flat per-tick rate at 30 ticks/sec compounds fast. Fixed by switching to a true per-second rate (`rate * dt`), at the user's suggestion, then re-verified with the same smoke-test methodology before training on it.
- **Bleed visual/reward both silently stop applying at the exact moment a cell matures**, even for a cell still critically dehydrated (observed moisture values as low as 0.1 in one traced case) — because `day_count` is one grid-wide counter, every still-`"growing"` cell flips to `"ready"` simultaneously at `DAYS_TO_MATURE`, and both the bleed check and the death check are gated on `status == "growing"`. This is a real, confirmed gap (traced via direct tick-by-tick instrumentation, not guessed), but left as-is per user decision — it's consistent with the pre-existing, intentional "ready freezes true state" design from Session 6, not a new bug introduced this session.

### Key decisions
- Death/bleed penalties were built as *team-level* hits (looping over all of `grid.agents`) rather than trying to attribute them to a single "responsible" agent — deliberately simple given the swarm shares one `TaskPolicy`; per-agent blame-tracking would add real complexity for no learning benefit under the current single-shared-brain architecture. Would need revisiting if agents ever get independent policies.
- Kept `DEATH_PENALTY` (sparse, terminal) *and* `DEHYDRATION_BLEED_RATE` (dense, continuous) as two separate mechanisms rather than replacing one with the other, per explicit user direction — the terminal penalty still matters for weeds/combined-cause deaths (no dense weed-equivalent exists yet, user has a separate weeds mechanism planned), the dense bleed specifically targets the dehydration credit-assignment problem.
- Chose not to fix the maturity-boundary mass-clear gap this session — explicitly a "leave it, flag it" decision from the user rather than an oversight.

### Dependencies
- `agent.py`: reads `DEATH_PENALTY`/`DEHYDRATION_BLEED_*` indirectly via `grid.py`, writes to `Agent.pending_penalty` — see `agent.md`.
- `rl_policy.py`: `_cell_features` now also imports `MOISTURE_DECAY_BASE`/`FPS` from `config.py` for the `est_moisture` feature — see `agent.md`.
- `eval_harvest_rate.py`: imports `NUM_AGENTS`, `FPS`, `POLICY_WEIGHTS_PATH` from `config.py`, `Grid`/`Agent`/`TaskPolicy` — same dependency shape as `train.py`, no new coupling introduced.

### [WORTH EXPLORING]
- No dense weed-equivalent to `DEHYDRATION_BLEED_RATE` exists yet — user has stated a separate weeds mechanism is planned, not yet designed or scheduled.
- The maturity-boundary mass-clear gap (see "What did not work") — revisit if it starts meaningfully misleading either the human watching or (more importantly) the reward signal itself, since a cell that survives to `"ready"` while critically neglected currently gets a completely free pass from that point on.
- `REPLAY_BUFFER_CAP=8000` was flagged (see `DEVLOG.md`) as a likely factor in the 300-episode plateau but never itself tested this session — a natural next lever if 160 episodes stops being enough.
